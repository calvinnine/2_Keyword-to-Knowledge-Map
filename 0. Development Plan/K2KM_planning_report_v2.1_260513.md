# 키워드 기반 연구구조 분석·아카이브 플랫폼 구축 기획보고서 v2.1

## Keyword-to-Knowledge Map (K2KM) + NTIS Comparative Layer

> 본 v2.1은 기존 v2 기획보고서에 **웹 그래프 시각화 구현 전략**, **Gephi 활용 범위**, **sigma.js + graphology 기반 웹 렌더링 구조**, **GEXF/JSON Export 전략**을 추가 반영한 개정안이다.
...
v2.1은 v2의 연구자 추천·품질 보정 모델을 유지하면서, 실제 웹서비스 구현 단계에서 필요한 **그래프 시각화 구현 전략**을 추가한다. 특히 Gephi 데스크톱 소스를 웹앱에 직접 이식하는 방식은 지양하고, K2KM 전용 웹 그래프 뷰어는 **sigma.js + graphology** 기반으로 구현하며, Gephi 생태계와의 호환성은 **GEXF/JSON Export**로 확보하는 방향을 반영한다.

## 0-2. 본문 반영 사항과 부록 분리 사항

| 구분 | 반영 위치 | 내용 |
| --- | --- | --- |
| 본문 반영 | 제10장~제18장 | 서비스 원칙, 연구자 추천 철학, 역할 라벨, 글로벌-국내 2축 비교, 웹 그래프 시각화 전략, UI/로드맵/리스크 반영 |
| 부록 반영 | 붙임 1~9 | 계산식, 가중치, DB 설계, API 설계, 상세 파이프라인, 추천 설명 템플릿, 주의 플래그, 그래프 시각화 구현 상세 |
